...
void GPS_init(void) {
    // 1. Set the Data Direction Register (DDR) and PORT
    
    // Set PD0 (RXD) as INPUT. We will receive data from the GPS TX pin here.
    DDRD &= ~(1 << DDD0); 
    // Enable the internal pull-up resistor on PD0 to keep the line stable
    PORTD |= (1 << PORTD0); 

    // Set PD1 (TXD) as OUTPUT. We will transmit data to the PC from here.
    DDRD |= (1 << DDD1);
